...
fun UpdateActivityScreen(
    viewModel: UpdateActivityViewModel,
    ...
    val tempActivity = viewModel.activity.observeAsState()

    val activity = remember { mutableStateOf(tempActivity.value ?: Activity(activityId, "", "other", LocalDate.now(), LocalTime.now(), LocalTime.now(), "")) }
    ...
    Column(
        ...
        val activityType = Types.typesList.find { it.name == activity.value.type }!!
        var selectedType by remember { mutableStateOf(activityType) }
        ...
        var selectedDate by remember { mutableStateOf(activity.value.date) }
        var selectedStartTime by remember { mutableStateOf(activity.value.startTime) }
        var selectedEndTime by remember { mutableStateOf(activity.value.endTime) }
